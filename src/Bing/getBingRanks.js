...
chrome.storage.sync.get('nPages', (items) => {
    ...
    try {
        ...
        results.forEach((result) => {
            ...
            const resultURL = result.querySelector('h2 > a').href;
            const publicationYear = result.querySelector('.news_dt') ?
                result.querySelector('.news_dt').innerText : '';
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.message === `ADD SCREENSHOT${currPage}`) {
        ...
        const b_skip_to_content = document.getElementById('b_skip_to_content');
        b_skip_to_content.style = 'display:none';
                ...
                fetch(`${uploadDestination}api/deposit/depositions/${depositId}/files`, {
                    ...
                    .then((data) => {
                        if (String(data.status).startsWith('4') || String(data.status).startsWith('5')) {
                            ...
                            //}
                            b_skip_to_content.style = 'display:';
                        }
                    })
                    .catch((error) => {
